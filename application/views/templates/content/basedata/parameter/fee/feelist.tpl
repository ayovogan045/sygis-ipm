    ...
    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
        <thead>
            <tr>
                ...
                <th>{$feewordinglabel}</th>
                <th>{$feeamountlabel}</th>
                <th>{$feetypelabel}</th>
                <th>{$actionlabel}</th>
            </tr>
        </thead>
        ...
        <tbody>
            ...
            {foreach from=$fee_datalist item=data}
                <tr>
                    ...
                    <td>{$data->getFee_type()->getWording()}</td>
                    <td>
                        <div class="hidden-sm hidden-xs action-buttons">
                            ...
                            <a id="edit-confirm" class="green tooltip-success" data-rel="tooltip" title="{$editlabel}"
                               href="#" 
                               onclick="edit_alert('{$root_url}/{$feeeditedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$fee}', '{$data->__toString()}');">
                                <i class="ace-icon fa fa-pencil bigger-130"></i>
                            </a>

                            <a id="delete-confirm" class="red tooltip-error" data-rel="tooltip" title="{$deletelabel}" href="#" 
                               onclick="delete_alert('{$root_url}/{$feedeletedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$fee}', '{$data->__toString()}');">
