{block name="content"}
    <div class="table-responsive">
        <table id="example" class="table table-bordred table-striped table-condensed">
            ...
            <tbody>
                {$index = 1}
                {foreach from=$posttypedatalist item=data}
                    <tr>
                        ...
                        <td>
                            <p data-placement="top" data-toggle="tooltip" title="">
                                <a href="#" onclick="updateAlert('{$base_url}{$posttypeeditedlink}/{$data->getId()}', '{$posttype}', '{$data->__toString()}');" class="btn btn-primary btn-sm" title="modifier" aria-label="Edit">
                                    <i class="fa fa-pencil fa-fw" aria-hidden="true"></i>
                                </a>
                                <a href="#" onclick="deleteAlert('{$base_url}{$posttypedeletedlink}/{$data->getId()}', '{$posttype}', '{$data->__toString()}');" class="btn btn-danger btn-sm" title="supprimer" aria-label="Delete" >
