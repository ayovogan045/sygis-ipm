    ...
    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
        <thead>
            <tr>
                ...
                <th>{$citywordinglabel}</th>
                <th>{$citycountrylabel}</th>
                <th>{$citydescriptionlabel}</th>
                <th>{$actionlabel}</th>
            </tr>
        </thead>
        ...
        <tbody>
            {$index = 1}
            {foreach from=$city_datalist item=data}
                <tr>
                    <td>{$index}</td>
                    <td class="center">
                        <label class="pos-rel">
                    ...
                    <td>
                        <div class="hidden-sm hidden-xs action-buttons">
                            ...
                            <a id="edit-confirm" class="green tooltip-success" data-rel="tooltip" title="{$editlabel}"
                               href="#" 
                               onclick="edit_alert('{$root_url}/{$cityeditedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$city}', '{$data->__toString()}');">
                                <i class="ace-icon fa fa-pencil bigger-130"></i>
                            </a>

                            <a id="delete-confirm" class="red tooltip-error" data-rel="tooltip" title="{$deletelabel}" href="#" 
                               onclick="delete_alert('{$root_url}/{$citydeletedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$city}', '{$data->__toString()}');">
