    ...
    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
        <thead>
            <tr>
                ...
                <th>{$countrywordinglabel}</th>
                <th>{$countryshortwordinglabel}</th>
                <th>{$countrycodelabel}</th>
                <th>{$countrynationalitylabel}</th>
                <th>{$actionlabel}</th>
            </tr>
        </thead>
        ...
        <tbody>
            ...
            {foreach from=$datalist item=data}
                <tr>
                    ...
                    <td>{$data->getShortWording()}</td>
                    <td>{$data->getCode()}</td>
                    <td>{$data->getNationality()}</td>
                    ...
                    <td>
                        <div class="hidden-sm hidden-xs action-buttons">
                            ...
                            <a id="edit-confirm" class="green tooltip-success" data-rel="tooltip" title="{$editlabel}"
                               href="#" 
                               onclick="edit_alert('{$root_url}/{$countryeditedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$country}', '{$data->__toString()}');">
                                <i class="ace-icon fa fa-pencil bigger-130"></i>
                            </a>

                            <a id="delete-confirm" class="red tooltip-error" data-rel="tooltip" title="{$deletelabel}" href="#" 
                               onclick="delete_alert('{$root_url}/{$countrydeletedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$country}', '{$data->__toString()}');">
