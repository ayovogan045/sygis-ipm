    ...
    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
        <thead>
            <tr>
                ...
                <th>{$num}</th>
                <th>{$specialitywordinglabel}</th>
                <th>{$specialitycodelabel}</th>
                <th>{$specialitymentionlabel}</th>
                <th>{$actionlabel}</th>
            </tr>
        </thead>
        ...
        <tbody>
            {$index = 1}
            {foreach from=$speciality_datalist item=data}
                <tr>
                    <td class="center">
                        <label class="pos-rel">
                            <input type="checkbox" class="ace" />
                    ...
                    <td>
                        <div class="hidden-sm hidden-xs action-buttons">
                            ...
                            <a id="edit-confirm" class="green tooltip-success" data-rel="tooltip" title="{$editlabel}"
                               href="#" 
                               onclick="edit_alert('{$root_url}/{$specialityeditedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$speciality}', '{$data->__toString()}');">
                                <i class="ace-icon fa fa-pencil bigger-130"></i>
                            </a>

                            <a id="delete-confirm" class="red tooltip-error" data-rel="tooltip" title="{$deletelabel}" href="#" 
                               onclick="delete_alert('{$root_url}/{$specialitydeletedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$speciality}', '{$data->__toString()}');">
