    ...
    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
        <thead>
            <tr>
                ...
                <th>{$num}</th>
                <th>{$classunitwordinglabel}</th>
                <th>{$classunitcodelabel}</th>
                <th>{$classunitlevellabel}</th>
                <th>{$actionlabel}</th>
            </tr>
        </thead>
                    ...
                    <td>
                        <div class="hidden-sm hidden-xs action-buttons">
                            ...
                            <a id="edit-confirm" class="green tooltip-success" data-rel="tooltip" title="{$editlabel}"
                               href="#" 
                               onclick="edit_alert('{$root_url}/{$classuniteditedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$classunit}', '{$data->__toString()}');">
                                <i class="ace-icon fa fa-pencil bigger-130"></i>
                            </a>

                            <a id="delete-confirm" class="red tooltip-error" data-rel="tooltip" title="{$deletelabel}" href="#" 
                               onclick="delete_alert('{$root_url}/{$classunitdeletedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$classunit}', '{$data->__toString()}');">
