    ...
    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
        <thead>
            <tr>
                ...
                <th>{$num}</th>
                <th>{$classroomlabel}</th>
                <th>{$classroomclassunitlabel}</th>
                <th>{$classroomgrouplabel}</th>
                <th>{$actionlabel}</th>
            </tr>
        </thead>
        ...
        <tbody>
            {$index = 1}
            {foreach from=$classroom_datalist item=data}
                <tr>
                    <td class="center">
                        <label class="pos-rel">
                            <input type="checkbox" class="ace" />
                    ...
                    <td>{$index}</td>
                    <td>{$data->__toString()}</td>
                    <td>{$data->getClassUnit()->__toString()}</td>
                    <td>{$data->getGroup()->__toString()}</td>
                    <td>
                        <div class="hidden-sm hidden-xs action-buttons">
                            ...
                            <a id="edit-confirm" class="green tooltip-success" data-rel="tooltip" title="{$editlabel}"
                               href="#" 
                               onclick="edit_alert('{$root_url}/{$classroomeditedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$classroom}', '{$data->__toString()}');">
                                <i class="ace-icon fa fa-pencil bigger-130"></i>
                            </a>

                            <a id="delete-confirm" class="red tooltip-error" data-rel="tooltip" title="{$deletelabel}" href="#" 
                               onclick="delete_alert('{$root_url}/{$classroomdeletedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$classunit}', '{$data->__toString()}');">
