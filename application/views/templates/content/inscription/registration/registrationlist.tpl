...
<div>
    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
        ...
        <tbody>
            {$index = 1}
            {foreach from=$registrationdatalist  item=data}
                <tr>
                    <td>{$index}</td>
                    <td class="center">
                        <label class="pos-rel">
                    ...
                    <td>
                        <div class="hidden-sm hidden-xs action-buttons">
                            ...
                            <a id="edit-confirm" class="green tooltip-success" data-rel="tooltip" title="{$editlabel}"
                               href="#" 
                               onclick="edit_alert('{$root_url}/{$registrationeditedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$candidat}', '{$data->__toString()}');">
                                <i class="ace-icon fa fa-pencil bigger-130"></i>
                            </a>

                            <a id="delete-confirm" class="red tooltip-error" data-rel="tooltip" title="{$deletelabel}" href="#" 
                               onclick="delete_alert('{$root_url}/{$registrationdeletedlink}/{$data->encryptionId($data->getId())}', '{$root_url}/{$candidat}', '{$data->__toString()}');">
