<?php

/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */

if (!defined('BASEPATH')) {
    exit('No direct script access allowed');
}

require_once APPPATH . 'controllers/BaseController.php';

use serviceImpl\GradeService,
    serviceImpl\LevelService,
    entities\Grade;

require_once APPPATH . 'models/serviceImpl/GradeService.php';
require_once APPPATH . 'models/serviceImpl/LevelService.php';

/**
 * @property CI_Form_validation fv a class instance of CI_Form_validation
 * It takes into account the constraints defined on each of the form field
 * It returns a warning message if a constraint is not checked
 * 
 * @The controller of Grade entity 
 * It serves as a bridge between the model and the view
 * 
 * @author Xlencia
 */
class GradeController extends BaseController {

    private $gradeService;
    private $levelService;
    private $grade_datalist;
    private $level_datalist;
    private $entity;
    private $wording;
    private $code;
    private $level;

    /**
     * constructor
     */
    public function __construct() {
        parent::__construct();

        ini_set('magic_quotes_gpc', 0);

        $this->layout->assignAll($this->config->item("gradeform"));

        $this->layout->assignOne('openlink', 'Donnée de Base');
        $this->layout->assignOne('activelink', 'Schoolparam');
        $this->layout->assignOne('subactivelink', 'Edition de grade');

        $this->gradeService = new GradeService();
        $this->levelService = new LevelService();
    }

    /**
     * Default function that will be executed unless another method specified
     * grade
     */
    public function grade() {
        $this->layout->assignOne('grade_datalist', $this->list_grade());
        $this->layout->assignOne('level_datalist', $this->levelChoiceListData());
        if ($this->session->userdata('done') === 1) {
            $this->layout->assignOne('success', "Suppression éffectuée avec succès");
        } else if ($this->session->userdata('done') === 0) {
            $this->layout->assignOne('error', "Suppression echouée");
        }
        $this->session->set_userdata('done', "");
        $this->editFields($this->session->userdata('grade_id'));
        // show the template
        $this->layout->view('content/basedata/schoolparam/grade/gradepage.tpl');
    }

    //add a new grade to the database
    public function add_grade() {
        // getting the differents values of fields
        $this->getPostData();

        //control wich operation can be done. Add or Edit
        if ($this->session->userdata('currentGrade') > 0) {
            $this->doUpdate($this->session->userdata('currentGrade'));
            $this->session->set_userdata('currentGrade', 0);
        } else {
            //create an grade object
            $grade = new Grade($this->getWording(), $this->getcode(), $this->getLevel(), $this->getNormalStatus());
            if ($this->gradeService->getOneExist($grade) != NULL) {
                $this->layout->assignOne('error', "un enrégistrement avec les mêmes informations existe déjà dans le système");
            } else {
                //proccess to add a new grade to database
                $this->crud->create($this->gradeService, $grade);
                $this->layout->assignOne('success', "Enrégistrement du grade " . $this->getWording() . ' éffectué avec succès ');
            }
        }
        $this->layout->assignOne('grade_datalist', $this->list_grade());
        $this->layout->assignOne('level_datalist', $this->levelChoiceListData());
        // show the template
        $this->layout->view('content/basedata/schoolparam/grade/gradepage.tpl');
    }

    //edit and update the status of the current grade data to the database
    public function edit_grade($grade_id = 0) {
        $this->entity = $this->crud->findOne($this->gradeService, $this->decryptionId($grade_id));
        if ($this->entity != NULL) {
            $this->session->set_userdata('grade_id', $this->entity->getId());
            $this->layout->assignOne('warning', "Vous êtes dans un processus de modification de donnée.  "
                    . "Etes vous sûr de vouloir continuer sinon <a href='" . base_url() . "index.php/grade'> abandonner</a> ");
            $this->grade();
        }
    }

    //update the status of the current grade data to the database
    public function delete_grade($grade_id = 0) {
        $done = $this->crud->deleteOne($this->gradeService, $this->decryptionId($grade_id), $this->getDeleteStatus());
        $this->session->set_userdata('done', $done);
        $this->grade();
    }

    //list the grade data from the database
    public function list_grade() {
        $this->grade_datalist = $this->crud->readSortAsc($this->gradeService, 'wording');
        return $this->grade_datalist;
    }

    //list of level to populate Level choicelist
    public function levelChoiceListData() {
        $this->level_datalist = $this->crud->readSortAsc($this->levelService, 'wording');
        return $this->level_datalist;
    }

    public function editFields($key) {
        if ($key > 0) {
            $this->entity = $this->crud->findOne($this->gradeService, $key);
            $this->keepFields($this->entity);
            $this->session->set_userdata('currentGrade', $this->entity->getId());
            $this->session->set_userdata('grade_id', 0);
        }
    }

    public function getPostData() {
        $this->setWording(htmlspecialchars($_POST['gradewording']));
        $this->setcode(htmlspecialchars($_POST['gradecode']));
        $this->setLevel($this->crud->findOne($this->levelService, htmlspecialchars($_POST['gradelevel'])));
    }

    public function keepFields($entity) {
        if ($entity !== NULL) {
            $this->layout->assignOne('gradewordingvalue', $entity->getWording());
            $this->layout->assignOne('gradecodevalue', $entity->getcode());
            $this->layout->assignOne('levelselected', $entity->getLevel());
        }
    }

    public function doUpdate($key) {
        $this->entity = $this->crud->findOne($this->gradeService, $key);
        $this->entity->setWording($this->getWording());
        $this->entity->setcode($this->getcode());
        $this->entity->setLevel($this->getLevel());
        if ($this->gradeService->getOneExist($this->entity) != NULL) {
            $this->layout->assignOne('error', "un enrégistrement avec les mêmes informations existe déjà dans le système");
            $this->session->set_userdata('currentGrade', 0);
        } else {
            $this->entity->setState($this->getUpdateStatus());
            $this->crud->updateOne($this->gradeService, $this->entity);
            $this->layout->assignOne('success', 'modification éffectuée avec succès ');
            $this->session->set_userdata('currentGrade', 0);
        }
    }

    function getWording() {
        return $this->wording;
    }

    function getcode() {
        return $this->code;
    }

    function getLevel() {
        return $this->level;
    }

    function setWording($wording) {
        $this->wording = $wording;
    }

    function setcode($code) {
        $this->code = $code;
    }

    function setLevel($level) {
        $this->level = $level;
    }

}
